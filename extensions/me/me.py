...
class Me(commands.Cog):
    ...
    async def me_get_role(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if await role_positions.get_bot_role_position(
            ...
            await inter.response.send_message(
                f"the bot doesn't have the highest role in the server"
            )
            return
    ...
    async def me_change_colour_from_rgb(
        self,
        inter: disnake.ApplicationCommandInteraction,
        red: commands.Range[0, 255] = None,
        green: commands.Range[0, 255] = None,
        blue: commands.Range[0, 255] = None,
        hex: str = None,
        random: bool = False,
    ):
        author = inter.author
        ...
        if role == None:
            await inter.response.send_message(
                "you don't have a role. use ```/me role``` to get one"
            )
            return
        if hex != None:
            await role.edit(
            ...
            await inter.response.send_message(
                f"changed your nickname colour to {role.color}"
            )
            return
        if random:
